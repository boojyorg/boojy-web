...
export function AudioPage() {
  ...
  const matchedPlatform = useMemo(() => {
    if (detected === 'linux') return null;
    return AUDIO_PLATFORMS.find((item) => item.id === detected && !('disabled' in item && item.disabled)) ?? null;
  }, [detected]);

  const [downloadHref, setDownloadHref] = useState(
    matchedPlatform && 'href' in matchedPlatform ? matchedPlatform.href : '#',
  );
  const [platformLabel, setPlatformLabel] = useState(
    matchedPlatform && 'shortLabel' in matchedPlatform ? matchedPlatform.shortLabel : 'Silicon',
  );
  const [selectedPlatform, setSelectedPlatform] = useState<string>(matchedPlatform?.id ?? 'mac-arm64');
  const [downloadIconHtml, setDownloadIconHtml] = useState(platformIconHtml(detected));
  const showFallback = !matchedPlatform;
  ...
  return (
    <>
      <section className="audio-hero">
        <div className="audio-hero-glow" aria-hidden="true" ref={glowRef} />
        <div className="container">
          <div className="audio-hero-title reveal">
            <img src="/images/boojy-logo.svg" alt="Boojy" className="audio-hero-logo" />
            <img src="/images/audio-text-logo.png" alt="Audio" className="audio-hero-product" />
          </div>
          <p className="audio-tagline reveal reveal-d1">Music production without the barriers.</p>
                ...
                'disabled' in platform && platform.disabled ? (
                  ...
                  >
                    ...
                    <span>
                      ...
                      {platform.pill ? <span className="platform-pill">{platform.pill}</span> : platform.label}
                    </span>
                  </span>
                ) : (
                  <a
      ...
      <section className="audio-screenshot-section">
        <div className="container">
          <div className="audio-screenshot-wrapper reveal reveal-d3">
            <img
              src="/images/screenshot-main-interface.png"
              alt="Boojy Audio"
              className="audio-screenshot"
              loading="lazy"
            />
          </div>
        </div>
        ...
        <div className="container">
          <div className="audio-features-grid">
            <div className="audio-feature-card">
              ...
              <p>No subscriptions, no paywalls. Free forever.</p>
            </div>
            <div className="audio-feature-card">
              <svg
                className="audio-feature-icon"
              ...
              <p>Use VST3 and AU instruments and effects.</p>
            </div>
            <div className="audio-feature-card">
              <svg
                className="audio-feature-icon"
              ...
              <p>Clean interface. No overwhelming menus.</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <ProductCloudCard
        cardClassName="audio-cloud-card"
        ctaClassName="audio-cloud-cta"
        description="Sync your projects across devices. Coming soon to Boojy Audio."
        cta={{ kind: 'text', label: 'Coming soon' }}
      />
    </>
  );
}
