...
/**
 * Interactive download CTA for /notes/. Server-renders the default (arm64) download so
 * the static HTML — including the version string — is complete; the client detects the
 * OS on mount and swaps in a direct download for the matched platform.
 */
export function NotesDownload({ versionText }: Props) {
  const { panelRef, toggleRef, toggle, close, panelClassName } = usePlatformsPanel();

  const [downloadHref, setDownloadHref] = useState(DEFAULT_DOWNLOAD);
  ...
  useEffect(() => {
    ...
    if (detected === 'linux' || detected === null) return;
